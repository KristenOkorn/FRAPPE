...
for n in range(len(pollutants)):
    ...
    for i in range(len(pods)):
        ...
        if pollutants[n] == 'O3':
            y = '$O_3$ (ppb)'
            titl = "INSTEP $O_3$ - {} ({})".format(pods[i],location[i])
        else:
            y = '$NO_2$ (ppm)'
            titl = "INSTEP $NO_2$ - {} ({})".format(pods[i],location[i])
            plt.ylabel(y)
        ...
        imgname = '{}_{}_{}_boxplot_hourly.png'.format(pods[i],location[i],pollutants[n])
