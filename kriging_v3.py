# -*- coding: utf-8 -*-
"""
Created on Wed Aug 23 14:41:58 2023

Saves output as video file instead of gif
(for easier point finding)
Also adds more whitespace to correct cut-off labels
...
for n in range(len(pollutants)):
    ...
    image_files = [filename for filename in os.listdir(path) if filename.endswith('.png')]

    #Sort the image filenames numerically based on the last part of the filename
    image_files.sort(key=lambda x: int(x.split('_')[-1].split('.')[0]))

    #Specify the output video filename
    video_filename = os.path.join(path, 'Kriging Interpolation - {}.avi'.format(pollutants[n]))
    ...
    out = cv2.VideoWriter(video_filename, fourcc, 20.0, frame_size)  # Adjust the frame rate as needed
