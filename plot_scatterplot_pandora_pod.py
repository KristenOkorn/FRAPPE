...
Scatterplot of INSTEP vs Pandora data
...
for n in range(len(pollutants)):
    ...
    for i in range(len(locations)):
        ...
        #add a trendline for each
        
        #add trendline
        z = np.polyfit(merge['pandora'], merge['pod'], 1) #calculate equation for trendline
        p = np.poly1d(z) #calculate points
        plt.plot(merge['pandora'], p(merge['pandora']),c=colors[i]) #add trendline to plot
        ...
        imgname = '{}_{}_scatterplot.png'.format(locations[i],pollutants[n])
